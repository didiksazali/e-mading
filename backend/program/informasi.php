<?php if ($_GET[act]==''){ ?> 
  <div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">     
    <div class="row">
      ...
      <hr><span class="" style="font-family:Calibri;color:green;font-size:18px;">Data Menu Informasi Website<a class='pull-right btn btn-primary btn-sm' href='index.php?view=konten&act=tambahinfo'>Tambahkan Data Informasi</a> <hr>
    ...
    <tbody>
      <?php 
        $tampil = mysql_query("SELECT * FROM informasi,tampil,kategori,status,baca where informasi.id_tampil=tampil.id_tampil AND informasi.id_kategori=kategori.id_kategori AND informasi.id_status=status.id_status AND informasi.id_baca=baca.id_baca order by id_informasi desc");
        $no = 1;
        ...
        echo "<tr><td>$no</td>
                ...
                if($_SESSION[level]=='admin'){
                  ...
                  <select class='form-control' name='id_tampil'> 
                                                                         ...
                                                                         echo "</select>
                    <button type='submit' class='btn btn-info btn-xs' href='#' name='ubah_tampil'><i class='glyphicon glyphicon-refresh'></i></button>
                    </form>"; }
                  echo"</td>
                <td>";
                if($_SESSION[level]=='admin'){
                  ...
                  <select class='form-control' name='id_status'> 
                                                                         ...
                                                                         echo "</select>
                    <button type='submit' class='btn btn-info btn-xs' href='#' name='ubah_status'><i class='glyphicon glyphicon-refresh'></i></button>
                    </form>"; }
                  echo"
                  </td>
                <td>";
                if($_SESSION[level]=='admin'){
                  ...
                  <select class='form-control' name='id_baca'> 
                                                                         ...
                                                                         echo "</select>
                    <button type='submit' class='btn btn-info btn-xs' href='#' name='ubah_baca'><i class='glyphicon glyphicon-refresh'></i></button>
                    </form>"; }
                  echo"</td>
                <td>
                <a class='btn btn-info btn-xs' title='Lihat Detail' href='?view=konten&act=detailinfo&id=$r[id_informasi]'><span class='glyphicon glyphicon-search'></span></a>
                <a class='btn btn-success btn-xs' title='Edit Data' href='?view=konten&act=editinfo&id=$r[id_informasi]'><span class='glyphicon glyphicon-edit'></span></a>
                <a class='btn btn-danger btn-xs' title='Delete Data' href='?view=konten&hapus=$r[id_informasi]'><span class='glyphicon glyphicon-remove'></span></a>
               </td>";
        echo "</tr>";
         $no++;
            }
  ...
  if (isset($_POST[tambah])){
      ...
      if ($filename != ''){      
        if (move_uploaded_file($_FILES['file']['tmp_name'], $uploadfile)) {
          mysql_query("INSERT INTO Informasi VALUES ('','$_POST[id_kategori]','$_POST[tanggal]','$_POST[jam]','$_POST[judul]',
            '$filename','$_POST[isi]','$_POST[id_tampil]','$_POST[id_status]','$_POST[id_baca]')");
             echo "<script>document.location='index.php?view=konten';</script>";
          
        }else{
          echo "<script>window.alert('Gagal Tambahkan Data.');
                      window.location='index.php?view=home'</script>";
        }
      }else {
        mysql_query("INSERT INTO informasi VALUES ('','$_POST[id_kategori]','$_POST[tanggal]','$_POST[jam]','$_POST[judul]','',
          '$_POST[isi]','$_POST[id_tampil]','$_POST[id_status]','$_POST[id_baca]')");
          ...
          <form method='POST' class='form-horizontal' action='' enctype='multipart/form-data'>
                <div class='col-md-12'>
                  ...
                  <tbody>
                    ...
                    echo "<input type='text' class='form-control' name='tanggal' value='$tgl' readonly><input type='hidden' class='form-control' name='jam' value='$jam' readonly>
                    ...
                    <tr><th scope='row'>Mode Tampilan Informasi</th>       <td><select class='form-control' name='id_tampil'>
                    <option value=''>----Pilih Mode Tampilan----- </option>";
                  $tampil=mysql_query("SELECT * FROM tampil ORDER BY nama_tampil");
                                      while($data=mysql_fetch_array($tampil))
                                      {
                                      
                                          echo "<option value='$data[id_tampil]'> $data[nama_tampil]</option>";
                                        }
                    echo "</td>
                    </select></tr>
